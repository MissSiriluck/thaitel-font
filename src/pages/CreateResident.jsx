  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (!createResident.residentTypeOf) {
        setCreateResidentError((curr) => ({ ...curr, residentTypeOf: 'กรุณาเลือกประเภทที่พักของท่าน' }))
      }
      if (!createResident.residentName) {
        setCreateResidentError((curr) => ({ ...curr, residentName: 'กรุณากรอกชื่อที่พักของท่าน' }))
      }
      ...
      if (!createResident.subDistrict) {
        setCreateResidentError((curr) => ({ ...curr, subDistrict: 'กรุณากรอกตำบล/แขวงของที่พักของท่าน' }))
      }
      if (!createResident.district) {
        setCreateResidentError((curr) => ({ ...curr, district: 'กรุณากรอกอำเภอ/เขตของที่พักของท่าน' }))
      }
      if (!createResident.province) {
        setCreateResidentError((curr) => ({ ...curr, province: 'กรุณากรอกจังหวัดของที่พักของท่าน' }))
      }
      ...
      if (!createResident.timeCheckInToStart) {
        setCreateResidentError((curr) => ({ ...curr, timeCheckInToStart: 'กรุณากรอกเวลาเช็คอินเริ่มต้นของที่พักของท่าน' }))
      }
      if (!createResident.timeCheckInToEnd) {
        setCreateResidentError((curr) => ({ ...curr, timeCheckInToEnd: 'กรุณากรอกเวลาเช็คอินสิ้นสุดของที่พักของท่าน' }))
      }
      if (!createResident.timeCheckOutToStart) {
        setCreateResidentError((curr) => ({ ...curr, timeCheckOutToStart: 'กรุณากรอกเวลาเช็คอินสิ้นสุดของที่พักของท่าน' }))
      }
      if (!createResident.timeCheckOutToEnd) {
        setCreateResidentError((curr) => ({ ...curr, timeCheckOutToEnd: 'กรุณากรอกเวลาเช็คอินสิ้นสุดของที่พักของท่าน' }))
      }
      ...
      if (!createResident.accName) {
        setCreateResidentError((curr) => ({ ...curr, accName: 'กรุณากรอกชื่อธนาคารของบัญชีของท่าน' }))
      }
