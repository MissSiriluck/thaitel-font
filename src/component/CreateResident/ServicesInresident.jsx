...
function ServicesInresident() {
  ...
  const setService = (index, newService) => {
    ...
    setCreateResident((cur) => ({ ...cur, serviceCollection: newServiceCollection }));
  };
    ...
    <Container maxWidth='md'>
      <Box>
        <Grid container direction='column'>
          <Grid item>
            <Typography sx={{ fontSize: '36px', marginBottom: '5px' }}>บริการภายในโรงแรม</Typography>
          </Grid>
          <Stack
            direction='column'
            spacing={2}
          ...
          >
            {createResident.serviceCollection.map((service, index) => {
              return <ServiceInResidentRow key={index} index={index} service={service} setService={setService} />;
            })}
          </Stack>
        </Grid>
      </Box>
